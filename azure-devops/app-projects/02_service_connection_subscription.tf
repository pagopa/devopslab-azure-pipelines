...
resource "azuredevops_serviceendpoint_azurerm" "LAB-SERVICE-CONN" {
  ...
  service_endpoint_name     = "${var.lab_subscription_name}-SERVICE-CONN"
  description               = "${var.lab_subscription_name} Service connection"
  azurerm_subscription_name = var.lab_subscription_name
